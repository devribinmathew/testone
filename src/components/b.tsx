...
const Box: React.FC = () => {
  ...
  const { isLoading: isLoadingUsers, error: errorUsers, data: users } = useQuery<
    Users[],
    Error
  >({
    queryKey: ["users"],
  ...
  const { isLoading: isLoadingPosts, error: errorPosts, data: posts } = useQuery<
    Posts[],
    Error
  >({
    queryKey: ["post"],
  ...
  useEffect(() => {
    if (!isLoadingUsers && !isLoadingPosts && !errorUsers && !errorPosts) {
      const updatedCombinedData: CombinedData[] = (users || []).map((user) => ({
        user,
        post: (posts || []).find((post) => post.userId === user.id),
      }));
      setCombinedData(updatedCombinedData);
    }
  }, [isLoadingUsers, isLoadingPosts, errorUsers, errorPosts, users, posts]);
